...
import com.bridgelabz.orders.utill.CartResponse;
...
public class OrderService implements IOrderService {
	 ...
	 */
	@Override
	public ResponseClass placeOrder(String token, List<Long> addressId, Long cartsId) {
		...
		if (isUserPresent) {
			...
			CartResponse isCartPresent = restTemplate.getForObject("http://cart-service/cart/cartCheck/" + cartsId,
					CartResponse.class);
			if (isCartPresent != null) {
				orderModel.setCartId(cartsId);
				orderModel.setOrderDate(LocalDate.now());
				orderModel.setBookId(isCartPresent.getCart().getBookId());
				orderModel.setQuantity(isCartPresent.getCart().getUserQuantity());
				orderModel.setPrice(isCartPresent.getCart().getTotalPrice());
